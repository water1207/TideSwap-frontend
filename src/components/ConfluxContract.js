  ...
  render () {
    ...
    if (selectedName) {
      if (this.state.selected.type === 'function') {
        ...
        buttonText = <span>Event <b>{selectedName}</b></span>//选中后
      }
    }
...
//负责根据传入的合约 abi 来渲染合约的信息，包括合约地址、合约方法和事件，合约提交的交互逻辑及显示执行后的结果
